...
class WC_Gateway_DOE_COD extends WC_Payment_Gateway {
    public $instructions;
    ...
    public function __construct() {
        $this->id = 'doe_cod';
        $this->icon = apply_filters('wc_doe_cod_icon', '');
        $this->has_fields = false;
        $this->method_title = __('Card Payment (doe_cod)', 'dynamic-order-emails');
        $this->method_description = __('Allow customers to pay via Card at checkout.', 'dynamic-order-emails');
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => __('Enable Card Payment', 'dynamic-order-emails'),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default' => __('Card Payment', 'dynamic-order-emails'),
                'desc_tip' => true,
            ),
            'description' => array(
...
<a href="mailto:{from_email}">{from_email}</a>. We\'re happy to assist!</p></div>', 'dynamic-order-emails'),
            ...
            'doe_card_initial_reminder_time' => array(
            ...
            'doe_card_initial_subject' => array(
                'title' => __('Card Initial Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'PAYMENT LINK : [{company_name}]: New order #{order_id}'
            ),
            'doe_card_reminder_24_time' => array(
            ...
            'doe_card_reminder_24_subject' => array(
                'title' => __('Card 24h Reminder Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'REMINDER : [{company_name}]: New order #{order_id}'
            ),
            'doe_card_reminder_48_time' => array(
            ...
            'doe_card_reminder_48_subject' => array(
                'title' => __('Card 48h Reminder Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'KINDLY REMINDER : [{company_name}]: New order #{order_id}'
            ),
            'doe_card_usa' => array(
                'title' => __('Card Email', 'dynamic-order-emails'),
                'type' => 'wpeditor',
                'default' => 'Hello {customer_name},<br><br>Thank you for placing a valuable order with us!!!<br><br>Your total payable amount is {order_total}<br><br>Please select a card payment option and include your order number #{order_id} in the payment details. Available options: Zelle, Venmo, Cash App, USDT, Remitly, Western Union.<br><br>Thank you for your cooperation. We appreciate your business.<br><br>Warm Regards,<br>Team {company_name}<br>Phone: +1 877-925-1112 (Call and Chat)<br><a href="https://wa.me/18779251112" target="_blank">WhatsApp us</a> (For chat only)',
            ...
            'enable_for_methods' => array(
                ...
                'css' => 'width: 400px;',
                'default' => '',
                'description' => __('If Card Payment is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'dynamic-order-emails'),
    ...
    public function process_payment($order_id) {
        ...
        if (!$this->is_available_for_shipping($order)) {
            wc_add_notice(__('Card Payment is not available for the selected shipping method.', 'dynamic-order-emails'), 'error');
            return;
        }
        $order->update_status('on-hold', __('Awaiting Card payment', 'dynamic-order-emails'));
    ...
    public function email_instructions($order, $sent_to_admin, $plain_text = false) {
		...
		// Only show header if order is on-hold or processing
