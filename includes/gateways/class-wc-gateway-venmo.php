...
class WC_Gateway_Venmo extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id = 'venmo_pay';
        $this->icon = apply_filters('wc_venmo_pay_icon', '');
        $this->has_fields = false;
        $this->method_title = __('Venmo Pay', 'dynamic-order-emails');
        $this->method_description = __('Allow customers to pay via Venmo at checkout.', 'dynamic-order-emails');
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => __('Enable Venmo Payment', 'dynamic-order-emails'),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default' => __('Venmo Pay', 'dynamic-order-emails'),
                'desc_tip' => true,
            ),
            'description' => array(
                ...
                'default' => __('Pay with Venmo upon order completion.', 'dynamic-order-emails'),
                'desc_tip' => true,
            ),
            'instructions' => array(
...
<a href="mailto:{from_email}">{from_email}</a>. We\'re happy to assist!</p></div>', 'dynamic-order-emails'),
            ...
            'doe_venmo_pay_usa_initial_reminder_time' => array(
            ...
            'doe_venmo_pay_usa_initial_subject' => array(
                'title' => __('Venmo Pay Initial Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'PAYMENT LINK : [{company_name}]: New order #{order_id}'
            ),
            'doe_venmo_pay_usa_reminder_24_time' => array(
            ...
            'doe_venmo_pay_usa_reminder_24_subject' => array(
                'title' => __('Venmo Pay 24h Reminder Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'REMINDER : [{company_name}]: New order #{order_id}'
            ),
            'doe_venmo_pay_usa_reminder_48_time' => array(
           ...
           'enable_for_methods' => array(
                ...
                'css' => 'width: 400px;',
                'default' => '',
                'description' => __('If Venmo Pay is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'dynamic-order-emails'),
            ...
            'enable_for_virtual' => array(
                ...
                'label' => __('Accept Venmo Pay if the order is virtual', 'dynamic-order-emails'),
                'type' => 'checkbox',
                'default' => 'yes'
            )
        );
    ...
    public function process_payment($order_id) {
        ...
        if (!$this->is_available_for_shipping($order)) {
            wc_add_notice(__('Venmo Pay is not available for the selected shipping method.', 'dynamic-order-emails'), 'error');
            return;
        }
        $order->update_status('on-hold', __('Awaiting Venmo payment', 'dynamic-order-emails'));
    ...
    /**
	 * Show header ONLY on 'on-hold' or 'processing'
	 */
	public function email_instructions($order, $sent_to_admin, $plain_text = false) {
		...
		if (!$content) {
			$this->log_debug("Venmo: No email content for order #{$order->get_id()}");
			return;
		}
