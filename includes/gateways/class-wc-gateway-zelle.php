...
class WC_Gateway_Zelle extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id = 'zelle_pay';
        $this->icon = apply_filters('wc_zelle_pay_icon', '');
        $this->has_fields = false;
        $this->method_title = __('Zelle Pay', 'dynamic-order-emails');
        $this->method_description = __('Allow customers to pay via Zelle at checkout.', 'dynamic-order-emails');
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => __('Enable Zelle Payment', 'dynamic-order-emails'),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default' => __('Zelle Pay', 'dynamic-order-emails'),
                'desc_tip' => true,
            ),
            'description' => array(
                ...
                'default' => __('Pay with Zelle upon order completion.', 'dynamic-order-emails'),
                'desc_tip' => true,
            ),
            'instructions' => array(
...
<a href="mailto:{from_email}">{from_email}</a>. We\'re happy to assist!</p></div>', 'dynamic-order-emails'),
            ...
            'doe_zelle_pay_usa_initial_reminder_time' => array(
            ...
            'doe_zelle_pay_usa_initial_subject' => array(
                'title' => __('Zelle Pay Initial Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'PAYMENT LINK : [{company_name}]: New order #{order_id}'
            ),
            'doe_zelle_pay_usa_reminder_24_time' => array(
            ...
            'doe_zelle_pay_usa_reminder_24_subject' => array(
                'title' => __('Zelle Pay 24h Reminder Email Subject', 'dynamic-order-emails'),
                'type' => 'text',
                'default' => 'REMINDER : [{company_name}]: New order #{order_id}'
            ),
            'doe_zelle_pay_usa_reminder_48_time' => array(
            ...
            'enable_for_methods' => array(
                ...
                'css' => 'width: 400px;',
                'default' => '',
                'description' => __('If Zelle Pay is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'dynamic-order-emails'),
            ...
            'enable_for_virtual' => array(
                ...
                'label' => __('Accept Zelle Pay if the order is virtual', 'dynamic-order-emails'),
                'type' => 'checkbox',
                'default' => 'yes'
            )
        );
    ...
    public function process_payment($order_id) {
        ...
        if (!$this->is_available_for_shipping($order)) {
            wc_add_notice(__('Zelle Pay is not available for the selected shipping method.', 'dynamic-order-emails'), 'error');
            return;
        }
        $order->update_status('on-hold', __('Awaiting Zelle payment', 'dynamic-order-emails'));
    ...
    /**
	 * Show email header ONLY on 'on-hold' or 'processing'
	 */
	public function email_instructions($order, $sent_to_admin, $plain_text = false) {
		...
		// Only show header on these statuses
		$allowed_statuses = $this->email_header_statuses ?? ['on-hold', 'processing'];
		...
		if (!$content) {
			$this->log_debug("Zelle Pay: No content for order #{$order->get_id()}");
			return;
		}
